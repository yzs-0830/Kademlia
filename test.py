...
def find_phase(pairs, phase_name):
    ...
    for (src, dst) in pairs:
        ...
        try:
            ...
            mc = c.call("count_msg")
            mc = int(mc.decode()) if isinstance(mc, bytes) else int(mc or 0)
            port = res.get("port") or res.get(b"port")
            if port == dst:
                correct_count += 1
            com_count += mc
            print(f"  → Result from {src}: {res} with {mc} msgs")
        except Exception as e:
            print(f"  → find_node error from {src} → {e}")
        time.sleep(2)
    print("Correct: ", correct_count, " over ", len(pairs))
    print("Average: ", com_count/len(pairs))
